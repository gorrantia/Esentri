...
/**
 * An immutable data structure representing HTTP status response bodies. It supports XML and JSON
 * representation. A JSON representation of this class would be something like the following:
 * 
 * <pre>
 *     {
 *         "status_code": 200,
...
public class DefaultResponse {
  ...
  /**
   * The ThirdPartyID from alusta basware.
   */
  private String id;
  ...
  /**
   * Private constructor to enforce the usage of a factory function to create new objects of this
   * class.
   * 
   * @param statusCode the status code as number
   * @param reason a simple message corresponding to the statusCode
   * @param message a human readable message
   * @param id the ThirdPartyID from alusta basware.
   * @param errors a list with detailed error descriptions
   */
  private DefaultResponse(int statusCode, String reason, String message, String id,
  ...
  /**
   * Static factory method to create a {@linkplain DefaultResponse} with a human readable message
   * and a given HTTP status code.
   * 
   * @param status the HTTP status code, like 200 or 400
   * @param message a human readable message
   */
  public static DefaultResponse createResponse(HttpStatus status, String message) {
    return new DefaultResponse(status.value(), status.getReasonPhrase(), message, null,
   ...
   * @param status the HTTP status code, like 200 or 400
   * @param message a human readable message
   * @param id ThirdPartyID from alusta basware.
   */
  public static DefaultResponse createResponse(HttpStatus status, String message, String id) {
    return new DefaultResponse(status.value(), status.getReasonPhrase(), message, id,
        Collections.emptyList());
  }

  /**
   * Static factory method to create a {@linkplain DefaultResponse} of type success with a human
   * readable message. It uses the HTTP status code 200 by default.
   * 
   * @param message a human readable message
   */
   ...
   * @param id ThirdPartyID from alusta basware.
   */
  public static DefaultResponse createSuccessResponse(String message, String id) {
    return createResponse(HttpStatus.OK, message, id);
  }

  /**
   * Static factory method to create a {@linkplain DefaultResponse} of type error with a human
   * readable message and a given HTTP status code.
   * 
   * @param status the HTTP status code, like 200 or 400
   * @param message a human readable message
   * @param errors a list with detailed error descriptions
   */
  public static DefaultResponse createErrorResponse(HttpStatus status, String message,
      List<ApiError> errors) {
    return new DefaultResponse(status.value(), status.getReasonPhrase(), message, null, errors);
  }
   ...
   * @param status the HTTP status code, like 200 or 400
   * @param message a human readable message
   * @param error a detailed error descriptions
   */
  public static DefaultResponse createErrorResponse(HttpStatus status, String message,
      ApiError error) {
  ...
  /**
   * Static factory method to create a {@linkplain DefaultResponse} of type error with a human
   * readable message. It uses the HTTP status code 500 by default.
   * 
   * @param message a human readable message
   * @param errors a list with detailed error descriptions
   */
  ...
  /**
   * Indicates whether this Response object is an error or a simple status message.
   * 
