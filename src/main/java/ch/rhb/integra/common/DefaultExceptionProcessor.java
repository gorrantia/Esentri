 ...
 * </p>
 * <p>
 * It builds a JSON response as <code>DefaultRestResponse</code> object.
 * Furthermore it sets the value 500 (internal server error) as HTTP status
 * code.
 * </p>
 *
...
public class DefaultExceptionProcessor implements org.apache.camel.Processor {
	...
	public void process(Exchange exchange) throws Exception {
		...
		DefaultResponse errorResponse = DefaultResponse.createGenericErrorResponse(ex.getMessage(), errors);

		exchange.getOut().setBody(errorResponse);
		exchange.getOut().setFault(true);
		exchange.getOut().setHeader(Exchange.HTTP_RESPONSE_CODE, HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
